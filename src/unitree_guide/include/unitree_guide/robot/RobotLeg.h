/*
* 单条腿的运动学/力映射封装类，RobotLeg 类封装了单条腿的运动学链、当前关节状态，以及常用的运动学/力学计算接口，包括正运动学、逆运动学、雅可比矩阵计算和力矩计算等。
* 这样做的好处是：
* 1. 代码复用：把所有与单条腿运动学/力学相关的计算都集中在一个类里，其他模块只需要调用这个类的接口，不需要重复实现相同的计算逻辑。
* 2. 代码清晰： 单条腿运动学/力学相关的计算逻辑都集中到一个类中，使得代码更加清晰和可读。 
*/
