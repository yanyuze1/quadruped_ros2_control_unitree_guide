/*
* 机器人原地站住不走，通过调机身姿态和高度来改变四条腿的目标落点，再用逆运动学求出关节角度，最后 PD 控制器把关节拉到目标角度上。这个状态的作用是让机器人在原地站住不动，同时允许用户通过遥控器微调机身姿态和高度来改变四条腿的目标落点，从而适应不同的地形或者躲避障碍物。
*/

#include "unitree_guide/FSM/StateFreeStand.h"
